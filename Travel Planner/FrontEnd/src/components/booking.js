  ...
  constructor(props) {
    ...
    this.state = {
      ...
      bookingForm: {
        ...
        date: this.props.location.state.checkInDate,
...
componentDidMount = (props) => {
  ...
  if(this.props.location.state.checkInDate !=='') {
    this.setState({ bookingFormValid: {buttonActive:true,noOfPersons:true, date: true}}, () => this.calculateCharges())
  }
  
  // this.setState({ bookingForm:{noOfPersons: noOfPersons, date: checkInDate, flights:this.props.location.state.flights }},
  //   ()=>this.calculateCharges());
}

calculateCharges = () => {
  console.log(this.state.deal.flightCharges);
  ...
  handleSubmit = (event) => {
    ...
    this.setState({show: true}, () => this.axiosCall())
  ...
  axiosCall = () => {
    ...
    axios.post(backendUrlBooking + "/" + sessionStorage.getItem('userId') + "/" + this.state.deal.destinationId, bookingDetails)
      .then(res =>{
          this.setState({bookingStatus: "Booking Confirmed!!", bookingSuccess: true, show: false})
      }).catch(err =>{
        this.setState({bookingStatus: err.response.data.message, bookingSuccess: true, bookingFail: true, show: false})
      })
  }
  ...
  bookingCard = () => {
   ...
   {console.log(this.state.bookingFormValid.noOfPersons,this.state.bookingFormValid.date)}
            ...
            {/* <div className="form-group"> */}
                ...
                <button id='goBack' className='btn btn-info btn ' type='button' onClick={() => this.setState({goBack: true})}>Go Back</button>
            {/* </div> */}
        </form>
      </CardContent>
    </Card></div>
  ...
  render(){  
    ...
    if(this.state.goBack && this.state.deal.destinationId[0]==='D'){      
      return  <Redirect to={'/packages/'+this.state.deal.continent}/>
    }else if(this.state.goBack && this.state.deal.destinationId[0]==='H'){
      return <Redirect to={'/hotDeals'}/>
    }else if(this.state.bookingSuccess) {
      return <div>{this.state.show?
