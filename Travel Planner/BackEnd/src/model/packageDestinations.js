...
packageDB.destination = ( continent ) => {
    ...
    return connection.getDestinationsCollection().then( ( collection ) => {
        return collection.find( { $or: [{continent: continent},{"details.itinerary.tourHighlights": continent}] } ).then( ( destinationObj ) => {
            if( destinationObj.length !== 0 ) {
                ...
                return connection.gethdCollection().then( hdcollection => {
                    return hdcollection.find( { $or: [{continent: continent},{"details.itinerary.tourHighlights": continent}]} ).then( hds => {
                        if( hds.length !== 0 ) {
